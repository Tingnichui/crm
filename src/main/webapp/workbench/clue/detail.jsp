	...
	$(function(){
		...
		$("#bundBtn").click(function (){
			...
			if ($xz.length==0){
				...
				if (confirm("确定关联以下数据吗？")) {
					var param = "cid=${clue.id}&"
					$.each($xz, function (index, element) {
						param += "aid=" + element.value
						if (index < $xz.length - 1) {
							param += "&"
						}
					})
					$.ajax({
						url: "workbench/activity/bund.do",
						data: param,
						type: "post",
						dataType: "json",
						success: function (data) {
...
<body>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10" style="width: 300px;">
                                <select class="form-control" id="edit-call">
                                    <option></option>
									<c:forEach items="${appellation}" var="s">
										<option value="${s.value}">${s.text}</option>
									</c:forEach>
                                </select>
                            </div>
	...
	<div style="position: relative; top: -70px;">
		<div style="position: relative; left: 40px; height: 30px;">
			...
			<div style="width: 300px;position: relative; left: 200px; top: -20px;"><b>${fullname.owner}${clue.appellation}</b></div>
